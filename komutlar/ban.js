...
var prefix = ayarlar.prefix;
  ...
  if (!user) return message.channel.send(`Sunucudan yasaklamak istediğiniz kullanıcıyı etiketlemelisiniz; \`${prefix}ban @kisi\` `);
 ...
 if (!message.guild.member(user).members.bannable) return message.channel.send(`Bu kişiyi sunucudan yasaklayamıyorum çünkü \`benden daha yüksek bir role sahip\` ya da \`bana gerekli yetkileri vermedin\`.`);
   ...
   if (!message.guild.member(user).members.bannable) return message.channel.send('Sunucudaki yetkilileri yasaklayamam!');

  message.guild.members.members.ban(user.id)
