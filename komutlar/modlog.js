  ...
  if(!modlogs) {
    ...
    message.channel.send(`> **Modlog kanalı başarılı bir şekilde ayarlandı.**`)
      ...
      if(modlogs) {
        ...
        const modlogkanal = message.guild.channels.cache.find(kanal => kanal.id === modlogs);
        return message.channel.send(`**Bu sunucuda daha önceden modlog kanalı ayarlanmış. Sıfırlamak için:** ||${prefix}modlog-sıfırla||\n**Ayarlanan kanal:** \`${modlogkanal.name}\``)
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['modlog','modlogayarla','mod-log-ayarla'],
    permLevel: 0
}
